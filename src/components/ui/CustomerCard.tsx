...
function formatRM(amount: number) {
  // simple formatting (you can replace with Intl later)
    ...
    <div className="rounded-xl border border-border bg-card p-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-lg bg-muted p-3 text-center">
          <div className="text-2xl font-semibold">{customer.totalRentals}</div>
          <div className="text-xs text-muted-foreground">Total Rentals</div>
        </div>

        <div className="rounded-lg bg-muted p-3 text-center">
          <div className="text-2xl font-semibold">{formatRM(customer.totalSpent)}</div>
          <div className="text-xs text-muted-foreground">Total Spent</div>
        </div>
      </div>

      {/* current rental box (only show if exists) */}
      {customer.currentRental ? (
        <div className="mt-4 rounded-lg border border-border bg-secondary p-3">
          ...
          <div className="mt-1 text-sm">{customer.currentRental.carName}</div>
